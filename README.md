...
For convenience, the mod also adds the interactions **Ask to Find Spouse**, **Find Someone to Invite to Court**, and **Find Guardian**, plus mid-game decisions for guardian automation and trait education.

The **Enable Automatic Guardian Appointments** decision makes searches started with **Find Guardian** immediately appoint the recommended guardian. A corresponding decision disables it again at any time. Birthday-triggered guardian suggestions still open normally, so the automation does not affect the mod's other events.

Education focuses can also be assigned automatically from traits. Use the **Configure Trait Education** decision to browse every trait currently loaded by the game, including traits from other mods, or start typing a displayed trait name or internal key to narrow the list. Clicking a trait immediately opens the gender choice, followed by the education-focus choice—there is no separate confirmation step. You can map all children, boys, or girls to diplomacy, martial, stewardship, intrigue, or learning. Boys and girls can have different mappings for the same trait. Choices are stored for the current ruler and immediately update eligible children under the player's care. The mappings can be changed or removed at any time and are reapplied on birthdays and when **Find Guardian** is used. If a child has multiple custom-mapped traits, the deterministic priority is diplomacy, martial, stewardship, intrigue, then learning.
...
The guardian automation and trait-based education decisions are player-only conveniences and do not affect AI rulers.
